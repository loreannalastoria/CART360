...
void live()
{ 
    //get the photocell frequency to have variations in sound
    offsetFrequency = getPhotoFrequency();
    testNote = analogRead(NOTE_IN_PIN);
    
    if (testNote > 8) {
      //play the note plus the frequency amount
    testNote = testNote + offsetFrequency;
    
    Serial.println (offsetFrequency);
...
void recordWithDuration()
{
  ...
  int theAdjuster = 30; //had to make this higher
...
{
  //IMPLEMENT
  //start the timer using millis
  startTime=millis(); 
  activeNoteButton = true;
  //set timePassed back to 0 when pressed
  timePassed = 0; 
...
void updateTimer()
{
  //time passed is the time passed in millis minus the orignial start time
...
void playCurrentNote()
{
   ...
   //same as live but adding the off set frequency and a delay
    tone(BUZZER_PIN,(analogRead(NOTE_IN_PIN) + offsetFrequency),100);
    //delay(100); // wait for tone to finish
...
{
 //IMPLEMENT
 //got this from powerpoint and changed variables
...
{
  //IMPLEMENT
  //loop to calculaye how long note is played and put into a array to play back
    for (int i = 0; i <= countNotes; i++) {
